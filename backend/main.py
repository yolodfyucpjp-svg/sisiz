...
app = FastAPI(title="Eco-Trace AI - ESG Calculation Engine", version="1.0.0", docs_url="/docs")
...
def process_analysis(data: AnalysisRequest) -> AnalysisResult:
    ...
    # Добавляем проценты в каждую категорию для generate_insights
    for cat in category_emissions:
        category_emissions[cat]["percentage"] = (category_emissions[cat]["kg"] / total_kg * 100) if total_kg > 0 else 0
    ...
    for cat, cat_data in category_emissions.items():
        if cat_data["kg"] > 0:
            co2_breakdown.append({
                "name": cat,
                "value": round(cat_data["percentage"], 1),
                "color": colors.get(cat, "#999"),
                "tons": round(cat_data["tons"], 2),
            })
...
def login(request: LoginRequest):
    ...
    token = jwt.encode({"sub": request.email, "exp": datetime.utcnow() + timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)}, SECRET_KEY, algorithm=ALGORITHM)
...
def register(request: RegisterRequest):
    ...
    USERS[request.email] = {'password': request.password, 'name': request.name or request.email.split('@')[0]}
    token = jwt.encode({"sub": request.email, "exp": datetime.utcnow() + timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)}, SECRET_KEY, algorithm=ALGORITHM)
...
def google_login(request: GoogleLoginRequest):
    try:
        idinfo = google_id_token.verify_oauth2_token(request.id_token, google_requests.Request())
        email = idinfo['email']
        name = idinfo.get('name', email.split('@')[0])
        if email not in USERS:
            USERS[email] = {'password': 'google_oauth', 'name': name}
        token = jwt.encode({"sub": email, "exp": datetime.utcnow() + timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)}, SECRET_KEY, algorithm=ALGORITHM)
...
def analyze_1c_data(request: AnalysisRequest):
...
async def upload_1c_excel(
    ...
    try:
        ...
        if 'item_name' not in df.columns or 'amount' not in df.columns:
            raise HTTPException(400, "Missing columns: item_name, amount")
        items = [OneCItem(item_name=str(row['item_name']), amount=float(row['amount']),
        ...
        req = AnalysisRequest(company_id=company_id, company_name=company_name, report_period=report_period,
                              revenue_millions=revenue_millions, expenses=items)
        result = process_analysis(req)
        analysis_storage[result.analysis_id] = result
        return result
    except Exception as e:
...
def download_report(analysis_id: str):
    ...
    story = [Paragraph("ESG Audit Report", ParagraphStyle('Title', fontSize=24, spaceAfter=30)),
             Spacer(1, 1*cm),
             Paragraph(f"Company: {result.company_name or result.company_id}", styles['Normal']),
             Paragraph(f"Report Period: {result.report_period}", styles['Normal']),
             Spacer(1, 1*cm),
             Paragraph(f"Green Rating: {result.green_rating}", ParagraphStyle(name='Rating', fontSize=18, textColor=colors.green)),
             Spacer(1, 1*cm)]
...
PRICE_IDS = {"pro": os.getenv("STRIPE_PRICE_PRO"), "enterprise": os.getenv("STRIPE_PRICE_ENTERPRISE")}
